...
-- library functions 
	...
	-- elements 
		...
		function library:image_grid(options)
			local cfg = {
				name          = options.name or nil,
				card_size     = options.card_size or dim2(0, 62, 0, 74),
			...
			-- outer wrapper (same pattern as slider/toggle)
			local grid_REAL = library:create("TextLabel", {
				Parent             = self.holder,
				Name               = "",
				FontFace           = library.font,
				TextColor3         = themes.preset.text,
			...
			-- optional name label (same style as label element)
			local name_offset = 0
			if cfg.name then
				name_offset = 14
				local left_comp = library:create("Frame", {
					Parent             = grid_REAL,
					Name               = "",
					BackgroundTransparency = 1,
					Position           = dim2(0, 2, 0, -1),
				...
				library:create("UIStroke", { Parent = name_lbl })
			end

			-- themed container: outline > inline > bg (gradient)
			local cont_outline = library:create("Frame", {
				Parent             = grid_REAL,
				Name               = "",
				Position           = dim2(0, 0, 0, name_offset),
				Size               = dim2(1, 0, 0, cfg.height),
				BorderColor3       = rgb(0, 0, 0),
				BorderSizePixel    = 0,
				BackgroundColor3   = themes.preset.outline
			})
			library:apply_theme(cont_outline, "outline", "BackgroundColor3")

			local cont_inline = library:create("Frame", {
				Parent             = cont_outline,
				Position           = dim2(0, 1, 0, 1),
				Size               = dim2(1, -2, 1, -2),
				BorderColor3       = rgb(0, 0, 0),
			...
			library:apply_theme(cont_inline, "inline", "BackgroundColor3")

			local cont_bg = library:create("Frame", {
				Parent             = cont_inline,
				Position           = dim2(0, 1, 0, 1),
				Size               = dim2(1, -2, 1, -2),
				BorderColor3       = rgb(0, 0, 0),
			...
			local cont_gradient = library:create("UIGradient", {
				Parent             = cont_bg,
				Rotation           = 90,
				Color              = rgbseq{rgbkey(0, rgb(41, 41, 55)), rgbkey(1, rgb(35, 35, 47))}
			})
			...
			function cfg:add_card(card_opts)
				local c_name     = card_opts.name  or ""
				local c_image    = card_opts.image or ""
				local c_callback = card_opts.callback or cfg.callback

				-- outline > inline > bg (gradient), same as every Atlanta element
				local c_outline = library:create("Frame", {
					Parent           = scroll,
					BorderColor3     = rgb(0, 0, 0),
					BorderSizePixel  = 0,
					BackgroundColor3 = themes.preset.outline
				})
				library:apply_theme(c_outline, "outline", "BackgroundColor3")

				local c_inline = library:create("Frame", {
					Parent           = c_outline,
					Position         = dim2(0, 1, 0, 1),
					Size             = dim2(1, -2, 1, -2),
					BorderColor3     = rgb(0, 0, 0),
				...
				library:apply_theme(c_inline, "inline", "BackgroundColor3")

				local c_btn = library:create("TextButton", {
					Parent           = c_inline,
					Position         = dim2(0, 1, 0, 1),
					Size             = dim2(1, -2, 1, -2),
					BorderColor3     = rgb(0, 0, 0),
				...
				local c_gradient = library:create("UIGradient", {
					Parent   = c_btn,
					Rotation = 90,
					Color    = rgbseq{rgbkey(0, rgb(41, 41, 55)), rgbkey(1, rgb(35, 35, 47))}
				})
				library:apply_theme(c_gradient, "contrast", "Color")

				-- image (fills top portion, leaves room for label)
				local c_img = library:create("ImageLabel", {
					Parent               = c_btn,
					Image                = c_image,
					Size                 = dim2(1, 0, 1, -14),
					Position             = dim2(0, 0, 0, 0),
				...
				local c_label = library:create("TextLabel", {
					Parent               = c_btn,
					FontFace             = library.font,
					TextColor3           = themes.preset.text,
					BorderColor3         = rgb(0, 0, 0),
				...
				library:create("UIStroke", { Parent = c_label })

				-- selected accent bar (top of card, hidden by default)
				local sel_bar = library:create("Frame", {
					Parent           = c_btn,
					Size             = dim2(1, 0, 0, 2),
					Position         = dim2(0, 0, 0, 0),
					BorderColor3     = rgb(0, 0, 0),
				...
				library:apply_theme(sel_bar, "accent", "BackgroundColor3")
				library:create("UIGradient", {
					Parent   = sel_bar,
					Rotation = 90,
					Color    = rgbseq{rgbkey(0, rgb(255, 255, 255)), rgbkey(1, rgb(167, 167, 167))}
				...
				table.insert(cfg.cards, card_data)

				c_btn.MouseButton1Click:Connect(function()
					-- deselect previous
					if cfg.selected and cfg.selected ~= card_data then
						cfg.selected.sel_bar.Visible = false
						cfg.selected.inline.BackgroundColor3 = themes.preset.inline
					end
					cfg.selected = card_data
					sel_bar.Visible = true
					c_inline.BackgroundColor3 = themes.preset.accent
					c_callback(c_name, c_image)
				end)

				return card_data
			end

			-- programmatically select a card by name
			function cfg:select_card(name)
				for _, card in next, cfg.cards do
					if card.name == name then
						if cfg.selected and cfg.selected ~= card then
							cfg.selected.sel_bar.Visible = false
							cfg.selected.inline.BackgroundColor3 = themes.preset.inline
						end
						cfg.selected = card
						card.sel_bar.Visible = true
						card.inline.BackgroundColor3 = themes.preset.accent
						return
					end
				end
			end
			...
			-- remove all cards
			function cfg:clear_cards()
				for _, card in next, cfg.cards do
					card.outline:Destroy()
				end
				cfg.cards = {}
				cfg.selected = nil
			end
			...
			function cfg.set_visible(bool)
				grid_REAL.Visible = bool
			end

			if options.flag then
				library.visible_flags[options.flag] = cfg.set_visible
			end
